    ...
    private int execute(CommandContext<CommandSourceStack> ctx, Collection<? extends Entity> entities, float amount, DamageSource damageSource) throws CommandSyntaxException {
        ...
        if (entities.size() == 1) {
            ...
            if (entity.hurtServer(serverLevel, damageSource, amount)) {
                componentBuilder.appendTranslatable("commands.helpfulcommands.dmg.other.single", Component.literal(String.valueOf(amount)).setStyle(textStyles.getPrimary()), StylingUtil.getAffectedEntityNameText(entity));
        ...
        componentBuilder.appendTranslatable("commands.helpfulcommands.dmg.other.multiple", Component.literal(String.valueOf(amount)).setStyle(textStyles.getPrimary()), StylingUtil.getAffectedEntitiesNumberText(affected));
