...
public class JsonIO<T> {
     ...
     * @param fileName      The name of the file (without extension).
     * @param typeClass     The class type of the data being handled.
     * @param builderConfig A {@link Consumer} that configures the {@link GsonBuilder}.
    ...
    public void updateBuffer(T data) {
        try {
            ...
            Constants.LOGGER.error("Failed to update JSON buffer for {}: {}", genericIO.getPath(), e.getMessage());
        }
    }
    ...
    public void flush(T data) {
        try {
            ...
            Constants.LOGGER.error("Failed to flush JSON to {}: {}", genericIO.getPath(), e.getMessage());
        }
    }
    ...
    public T getData() {
        try {
            ...
            Constants.LOGGER.error("Failed to parse JSON from {}: {}", genericIO.getPath(), e.getMessage());
            return null;
        }
    }
}
