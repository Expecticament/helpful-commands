...
public class ExplosionCommand extends HelpfulCommandsCommand {
    private static final Dynamic2CommandExceptionType POWER_CONFIG_VALUE_EXCEEDED = new Dynamic2CommandExceptionType((src, maxPower) ->
            new ComponentBuilder((CommandSourceStack) src).appendTranslatable("commands.helpfulcommands.explosion.power_config_value_exceeded", Component.literal(String.valueOf(maxPower)).setStyle(StylingManager.getCurrentStyle().getTextStyles().getPrimary())).build()
    );
    ...
    private int executePosition(CommandContext<CommandSourceStack> ctx, Vec3 position, Integer power, boolean destroyBlocks) throws CommandSyntaxException {
        ...
        componentBuilder.appendTranslatable("commands.helpfulcommands.explosion", StylingUtil.getLocationText(position, ServerLevelUtil.getLevelLocation(serverLevel)));
