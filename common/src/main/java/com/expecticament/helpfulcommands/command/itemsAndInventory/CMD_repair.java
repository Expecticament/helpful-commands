...
public class CMD_repair extends HelpfulCommandsCommand {

    public CMD_repair(ModCommandManager.CommandData commandData) {
        super(commandData);
    }
    ...
    private int execute(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> players) throws CommandSyntaxException {
        ...
        if (targets.isEmpty() || (targets.size() == 1 && sourcePlayer != null && targets.contains(sourcePlayer))) {
            ...
            repairAndSendMessage(sourcePlayer, src, textStyles, commandFeedback, true, true);
        ...
        for (ServerPlayer player : targets) {
            ItemStack itemStack = repairAndSendMessage(player, src, textStyles, commandFeedback, player == sourcePlayer, false);
        ...
        textBuilder
                .appendTranslatable("commands.helpful_commands.repair.output.success.other.part1")
                .appendWhitespace();
        ...
        if (affected.size() == 1) {
            ...
            textBuilder
                    ...
                    .appendTranslatable("commands.helpful_commands.repair.output.success.other.part2.single")
                    .appendWhitespace()
            ...
            textBuilder
                    ...
                    .appendWhitespace()
                    .appendTranslatable("commands.helpful_commands.repair.output.success.other.part2.multiple");
        }
    ...
    private ItemStack repairAndSendMessage(ServerPlayer player, CommandSourceStack commandSourceStack, HelpfulCommandsStyle.TextStyles textStyles, boolean commandFeedback, boolean isSource, boolean feedbackSource) {
        ItemStack itemStack = player.getMainHandItem();
        if (!itemStack.isEmpty() && itemStack.isDamageableItem() && itemStack.isDamaged()) {
            itemStack.set(DataComponents.DAMAGE, 0);

            Component itemCustomNameComponent = itemStack.getCustomName();
            String itemName = (itemCustomNameComponent != null ? itemCustomNameComponent.getString() : itemStack.getItemName().getString());
            ...
            if (commandFeedback) {
                if (isSource) {
                    if (feedbackSource) {
                        ...
                        textBuilder
                                .appendTranslatable("commands.helpful_commands.repair.output.success.self")
                                .appendWhitespace()
                    ...
                    textBuilder.setStyle(textStyles.getAffectedPositive());
                    textBuilder
                            .appendTranslatable("commands.helpful_commands.repair.output.success.affected.part1")
                            .appendWhitespace()
                            .appendComponent(itemNameComponent)
                            .appendWhitespace()
                            .appendTranslatable("commands.helpful_commands.repair.output.success.affected.part2");
