...
public class FlyCommand extends HelpfulCommandsCommand {
        ...
        dispatcher.register(Commands.literal(modCommand.getName())
                ...
                .then(Commands.argument("state", BoolArgumentType.bool())
                        ...
                        .then(Commands.argument("players", EntityArgument.players())
                                .requires(src -> PermissionsUtil.hasPermission(src, ModPermissions.Permission.COMMAND_FLY_OTHERS))
    ...
    protected boolean checkBaseCommandRequirements(CommandSourceStack source) {
        return PermissionsUtil.hasPermission(source, ModPermissions.Permission.COMMAND_FLY);
    }
    ...
    private int executeSelf(CommandContext<CommandSourceStack> ctx, Boolean state) throws CommandSyntaxException {
        ...
        componentBuilder.appendTranslatable("commands.helpfulcommands.fly.self", Component.literal(TranslationManager.translate(src, "commands.helpfulcommands.fly." + String.valueOf(mayFly).toLowerCase())));
        componentBuilder.setStyle(mayFly ? textStyles.getAffectedPositive() : textStyles.getAffectedNegative());
    ...
    private int executeOther(CommandContext<CommandSourceStack> ctx, Boolean state, Collection<ServerPlayer> players) throws CommandSyntaxException {
                ...
                .peek(player -> {
                    if (commandFeedback) {
                        if (player != sourcePlayer) {
                            ...
                            componentBuilder.appendTranslatable("commands.helpfulcommands.fly.affected", Component.literal(TranslationManager.translate(src, "commands.helpfulcommands.fly." + String.valueOf(state).toLowerCase()))).setStyle(textStyles.getAffectedNeutral());
        ...
        if (affected.size() == 1) {
            ...
            affectedText
                    ...
                    .append(" ")
                    .append(TranslationManager.translate(src, "commands.helpfulcommands.fly.other.multiple"));
        }

        ComponentBuilder componentBuilder = new ComponentBuilder(src);
        componentBuilder.appendTranslatable("commands.helpfulcommands.fly.other", Component.literal(TranslationManager.translate(src, "commands.helpfulcommands.fly." + String.valueOf(state).toLowerCase())), affectedText).setStyle(textStyles.getSuccess());
