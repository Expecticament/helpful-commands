...
public class JumpCommand extends HelpfulCommandsCommand {
    private static final Dynamic2CommandExceptionType DISTANCE_CONFIG_VALUE_EXCEEDED = new Dynamic2CommandExceptionType((src, maxDistance) ->
            new ComponentBuilder((CommandSourceStack) src).appendTranslatable("commands.helpfulcommands.jump.error.distance_config_value_exceeded", Component.literal(String.valueOf(maxDistance)).setStyle(StylingManager.getCurrentStyle().getTextStyles().getPrimary())).build()
    );
    ...
    private int execute(CommandContext<CommandSourceStack> ctx, Double distance, boolean checkBlocks) throws CommandSyntaxException {
        ...
        componentBuilder.appendTranslatable("commands.helpfulcommands.jump", StylingUtil.getPositionText(safePos));
