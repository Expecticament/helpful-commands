...
public class PermissionsUtil {
    ...
    public static <T> T getMetaOrElseConfigValue(ServerPlayer player, ConfigManager.CONFIG_FIELD field) {
        ...
        try {
            ...
            String meta = net.luckperms.api.LuckPermsProvider.get().getPlayerAdapter(ServerPlayer.class).getMetaData(player).getMetaValue(Constants.MOD_SHORT_ID + "_" + field.toString().toLowerCase());
            if (meta == null) {
                return config.readField(field);
            }
