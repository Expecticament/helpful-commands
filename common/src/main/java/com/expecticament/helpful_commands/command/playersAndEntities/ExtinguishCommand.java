...
public class ExtinguishCommand extends HelpfulCommandsCommand {
    ...
    public void register(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext buildContext, Commands.CommandSelection commandSelection) {
        ...
        dispatcher.register(Commands.literal(modCommand.getName())
                ...
                .then(Commands.argument("entities", EntityArgument.entities())
                        .requires(src -> PermissionsUtil.hasPermission(src, ModPermissions.Permission.COMMAND_EXTINGUISH_OTHERS))
                        .executes(ctx -> executeOther(ctx, EntityArgument.getEntities(ctx, "entities")))
                )
        );
    }
    ...
    protected boolean checkBaseCommandRequirements(CommandSourceStack source) {
        return PermissionsUtil.hasPermission(source, ModPermissions.Permission.COMMAND_EXTINGUISH);
    }
    ...
    private int executeSelf(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!extinguish(sourcePlayer)) {
            return 0;
        }

        componentBuilder.appendTranslatable("commands.helpful_commands.extinguish.self");
    ...
    private int executeOther(CommandContext<CommandSourceStack> ctx, Collection<? extends Entity> entities) throws CommandSyntaxException {
        CommandSourceStack src = ctx.getSource();
        ...
        List<? extends Entity> affected = entities.stream()
                .filter(this::extinguish)
                .peek(entity -> {
                    if (entity.isAlwaysTicking() && commandFeedback) {
                        ServerPlayer player = (ServerPlayer) entity;
                        if (player != sourcePlayer) {
                            ComponentBuilder componentBuilder = new ComponentBuilder(player);
                            componentBuilder.appendTranslatable("commands.helpful_commands.extinguish.affected").setStyle(textStyles.getAffectedPositive());
        ...
        if (affected.size() == 1) {
            ...
            affectedText
                    ...
                    .append(" ")
                    .append(TranslationManager.translate(src, "commands.helpful_commands.extinguish.other.multiple"));
        }

        ComponentBuilder componentBuilder = new ComponentBuilder(src);
        componentBuilder.appendTranslatable("commands.helpful_commands.extinguish.other", affectedText).setStyle(textStyles.getSuccess());
    ...
    private boolean extinguish(Entity entity) {
